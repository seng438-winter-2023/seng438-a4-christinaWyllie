...
**6. intersects()157:** Negated double field lower (killed)\ 
This mutation takes the value lower and negates it. The initialized original range is from -5-5. By negating the lower bound value the range becomes 5-5. The test intersectWithinRange catches this mutation because the range passed in is -4.99-4.99 and should return true. However, because the mutation changed the range to 5-5, -4.99-4.99 no longer returns true.

**7. contains()144:** replaced boolean return with true (KILLED)\
This mutation replaces the return value in the function returns from something calcuated by the program with "true". This mutation is caught by rangeContainsPastLowerBound() because that test sends in a value that should have the function originally return false. When the mutation returns true this test will fail.\

**8. contains()144:** negated double field lower (KILLED)\
This mutation causes the variable lower to be be negated. For example, if the value is -5 than it will be negated to 5. This mutation is caught by the test rangeContainsLowerBound. The range set in the test is from -1 to 1. Therefore, negating lower bound means the range is from 1 to 1. However by testing if the true lower bound is contained (-1) we catch this because it will no longer return true with the mutation.\

**9. intersects()161:** Incremented (a++) double local variable 1 (SURVIVED)\
This mutation occurs by munipulating the local variable b0 and incrementing the value. The test survived because there is no test case that considers the boundary between bo < this.upper. If a test case were to be created to kill this mutation it would have to make sure b0 is one less than this.upper and b1 >= b0 regardless of being incremented or not.\ **CHECK THIS

**10. getLowerBound()105:** Replaced double return with 0.0d (KILLED)\
This mutation takes the function getLowerBound() and munipulates the precision of this.lower to be 0.0d. This is caught by both the lower bound tests because they have a specified precision of .000000001d. 0.0d would not be as accurate, thus the mutation gets killed.\
...
![alt text](https://github.com/seng438-winter-2023/seng438-a4-christinaWyllie/blob/main/Images/ImprovedMutationCoverageDataUtilTest.png)
...
Together these tests increased the mutation coverage from 45% to 67%.
